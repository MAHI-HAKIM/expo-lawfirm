...
interface CaseAndLawyerStepProps {
  caseTypes: CaseType[];
  lawyers: Lawyer[];
...
export function CaseAndLawyerStep({
  caseTypes,
  lawyers,
  onSelectCaseType,
  onSelectLawyer,
  ...
  const filteredLawyers = lawyers.filter((lawyer) => {
    ...
    // If a case type is selected, filter by specialization
    if (selectedCaseTypeId) {
      const selectedCaseType = caseTypes.find(
        (ct) => ct.id === selectedCaseTypeId
      );
      if (selectedCaseType) {
        // Map case type to specialization
        const specializationMap: { [key: string]: string } = {
          "Criminal Cases": "Criminal Law",
          "Family Cases": "Family Law",
          "Corporate Cases": "Corporate Law",
          "Real Estate Cases": "Real Estate Law",
          "General Cases": "General Practice",
        };

        const requiredSpecialization = specializationMap[selectedCaseType.name];
        return (
          matchesSearch && lawyer.specialization === requiredSpecialization
        );
      }
    }
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#2e2d2d] p-3 rounded-xl">
          ...
          <CardContent className="pt-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="flex items-start space-x-2 p-3 rounded-md border border-[#4d3f29]/40 bg-[#383838]"
                >
                  <Skeleton className="h-4 w-4 mt-1 bg-[#2a251e]" />
        ...
        {/* Lawyer Selection Loading Skeleton */}
        <Card className="border-0 shadow-lg rounded-lg overflow-hidden bg-[#21201f]">
          <CardHeader className="bg-[#2a251e] border-b border-[#4d3f29]/30">
            <Skeleton className="h-8 w-2/3 bg-[#2a251e]" />
            <Skeleton className="h-4 w-full bg-[#2a251e]" />
      ...
      <Card className="border-0 shadow-lg rounded-lg overflow-hidden bg-[#21201f] dark:bg-[#2d2823]">
        ...
        <CardContent className="pt-5">
          <RadioGroup
            ...
            onValueChange={(value) => onSelectCaseType(parseInt(value, 10))}
          ...
          >
            {caseTypes.map((caseType) => (
              <div
                ...
                onClick={() => onSelectCaseType(caseType.id)}
              >
                <RadioGroupItem
              ...
              {filteredLawyers.length === 0 ? (
                <div className="p-4 text-center border border-dashed rounded-md border-[#4d3f29]/60 bg-[#2a251e]/50">
                  <p className="text-amber-100/60">
                    No attorneys available for this case type.
                  </p>
                </div>
              ) : (
                <RadioGroup
